...
-- 01. Liste o nome de cada projeto.1,0,1,2,0,1,1,1
-- Para cada projeto, conte quantas tarefas existem para cada nível de `prioridade`
-- ('Baixa', 'Normal', 'Alta', 'Urgente'). 
-- O resultado deve mostrar o nome do projeto e 
-- colunas separadas para a contagem de cada prioridade. 
-- Projetos sem tarefas de uma certa prioridade devem mostrar 0.
...
ORDER BY
	p.nome;
-- pode ser feita com count e sem o else tbm

select * from tarefas;

-- 02.Mostre o nome dos responsáveis e a quantidade de tarefas
-- pelas quais eles são responsáveis (id_responsavel_tarefa).
-- Liste apenas os responsáveis que têm 3 ou mais tarefas atribuídas.
-- Ordene do mais atarefado para o menos atarefado.

SELECT 
	r.nome AS "Nome do Responsável",
	COUNT(t.id) AS "Quantidade de Tarefas"
FROM
	Responsaveis r
JOIN
...
GROUP BY
	r.nome
HAVING -- o HAVING não olha para pessoas individuais, mas para as características do grupo.
	COUNT(t.id) >= 3
ORDER BY
	"Quantidade de Tarefas" DESC;
    
-- 03.Calcule a "duração percebida" média das tarefas (em dias) para projetos, 
-- agrupados pelo status do projeto ('Pendente', 'Em Andamento', 'Concluído', 'Cancelado'). 
-- Para este exercício, considere a "duração percebida"
-- de uma tarefa como a diferença em dias entre a data_prevista_entrega 
-- da tarefa e a data_inicio do projeto ao qual ela pertence. 
-- Exiba o status do projeto e a respectiva duração percebida média das tarefas.
-- Arredonde a média para duas casas decimais.
-- Ignore tarefas ou projetos onde as datas necessárias sejam nulas.

SELECT
    p.status AS "Status do Projeto",
...
JOIN
    Tarefas t ON p.id = t.id_projeto
WHERE
    t.data_prevista_entrega IS NOT NULL AND p.data_inicio IS NOT NULL
GROUP BY
    p.status
ORDER BY
    p.status;
...
-- 04.Para cada projeto que esteja atualmente com o status 'Em Andamento',
-- liste o nome do projeto. Dentro de cada um desses projetos, 
-- mostre cada cargo (da tabela Responsaveis) dos responsáveis que 
-- possuem tarefas atribuídas nesse projeto específico,
-- e o número de tarefas que responsáveis com aquele cargo têm nesse projeto.
...
-- 05. Gere um ranking com os Top 5 projetos com mais tarefas em atraso.
SELECT
    p.nome AS "Nome do Projeto",
    COUNT(t.id) AS "Tarefas em Atraso"
FROM
    Projetos p
JOIN
    Tarefas t ON p.id = t.id_projeto
...
GROUP BY
    p.nome
ORDER BY
    "Tarefas em Atraso" DESC
LIMIT 5;

-- 06. Verifique quais as Tarefas são urgentes/altas e não concluídas
SELECT
    *
FROM
    Tarefas
...
-- 07. Listar projetos com uma ou mais tarefas com status 'Pendente'
SELECT DISTINCT
    p.id,
    p.nome,
    p.status
