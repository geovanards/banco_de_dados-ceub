...
/* 01.Crie uma consulta SQL que exiba o nome do participante, 
...
/* 02.Escreva uma consulta que exiba o nome do evento, 
...
/*03.Liste todos os participantes com uma coluna extra chamada tipo_participante, que deve conter:
...
/* 04.Crie uma consulta que retorne o nome dos organizadores, seus cargos e uma nova coluna chamada funcao, com:
...
/* 05.Crie uma consulta que exiba o nome dos participantes e uma coluna chamada prioridade_contato, que deve mostrar:
...
/* 06.Liste todos os eventos com nome, tipo e uma nova coluna chamada classificacao_tipo, que mostre:
...
/*01- Liste os eventos que têm participantes com pagamento pendente
 e exiba apenas aqueles com mais de 1 pendência registrada.*/
 
SELECT nome_evento, COUNT(*) as qtd_pendencias
FROM Eventos 
JOIN Inscricoes on id_evento 
...
/* 02- Exiba os organizadores e a quantidade de eventos organizados por cada um,
 mostrando apenas aqueles que organizaram mais de 1 evento.*/
SELECT organizador_id, COUNT(*) as total_eventos 
FROM Eventos 
GROUP BY organizador_id 
HAVING COUNT(*) > 1;


/* 03- Mostre os nomes dos participantes e a quantidade de eventos em que estão inscritos, 
 limitando o resultado apenas aos que têm mais de uma inscrição.*/
SELECT p.nome, count(*) as total_inscricoes 
from Inscricoes i
JOIN Participantes p on i.id_participante = p.id
...
/* 04- Liste os eventos que têm mais da metade dos inscritos com pagamento pendente. 
Para cada evento, exiba a quantidade de pendências e o total de inscrições.*/
SELECT 
	Eventos.nome_evento,
    COUNT(Inscricoes.id) AS total_inscricoes,
    SUM(CASE WHEN Inscricoes.status_pagamento = 'Pendente' THEN 1 ELSE 0 END) AS pendencias
FROM Eventos

	JOIN Inscricoes ON Eventos.id = Inscricoes.id_evento
    GROUP BY Eventos.id, Eventos.nome_evento
HAVING SUM(CASE WHEN Inscricoes.status_pagamento = 'Pendente' THEN 1 ELSE 0 END) > COUNT(Inscricoes.id)/2;
